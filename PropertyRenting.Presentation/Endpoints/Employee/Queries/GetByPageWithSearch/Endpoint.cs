...
internal sealed class Endpoint : Endpoint<Request, PagedList<EmployeeDTO>>
{
    ...
    public override void Configure()
    {
        ...
        AllowAnonymous();
        Group<EmployeeGroup>();
        Description(x => x.WithName("GetEmployeesByPageWithSearch").Produces<PagedList<EmployeeDTO>>(200).ProducesValidationProblem()
        .Produces(204).ProducesProblemFE<InternalErrorResponse>(500));
        Version(1);
    }
    public override async Task HandleAsync(Request request, CancellationToken cancellationToken)
    {
        var result = await _sender.Send(new GetEmployeesByPageWithSearchQuery(request.Search, request.Page, request.PageSize), cancellationToken);
