...
public abstract class WriteRepository<TEntity> : IWriteRepository<TEntity> where TEntity : AggregateRoot
{
    ...
    public WriteRepository(PropertyRentingWriteContext context)
    {
    ...
    public async Task<TEntity> GetEntityByIdAsync(EntityId id, CancellationToken cancellationToken = default) => await EntitySet.Where(x => x.Id == id).SingleOrDefaultAsync(cancellationToken);
    public void Create(TEntity entity) => EntitySet.Add(entity);
    public void CreateList(List<TEntity> entities) => EntitySet.AddRange(entities);
    public void Update(TEntity entity) => EntitySet.Update(entity);
    public void Delete(TEntity entity) => EntitySet.Remove(entity);
    public void DeleteList(List<TEntity> entities) => EntitySet.RemoveRange(entities);
